...
document.querySelectorAll(".petgrooming-magnet-item").forEach((item) => {
  const icon = item.querySelector(".petgrooming-magnet-item-svg");
  let bounds;
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    initTextTicker('#petgrooming-text-marquee');

});


// Petgrooming project showcase animation cursor follow
document.addEventListener("DOMContentLoaded", () => {
    ...
    const grid = document.getElementById("petgrooming-works-grid");
    if (!grid) return;

    // Cards Reveal
    gsap.from(grid.querySelectorAll(".petgrooming-work-card"), {
        y: 60,
        opacity: 0,
        duration: 0.9,
        stagger: 0.15,
    ...
    const medias = [...grid.querySelectorAll(".petgrooming-work-media")];

    medias.forEach((media) => {

        const btn = media.querySelector(".petgrooming-view-work");
        if (!btn) return;
...
// Petgrooming testimonial splide js
document.addEventListener('DOMContentLoaded', function () {
  if (document.querySelector('#petgrooming-testimonial-slider')) {

    new Splide('#petgrooming-testimonial-slider', {
      type: 'loop',
      ...
      classes: {
        arrows: 'splide__arrows petgrooming-arrows',
        arrow: 'splide__arrow petgrooming-arrow',
        prev: 'splide__arrow--prev petgrooming-arrow-prev',
        next: 'splide__arrow--next petgrooming-arrow-next',
      },
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const faqItems = document.querySelectorAll(".petgrooming-faq-item");


  faqItems.forEach((item) => {

    const trigger = item; // Entire FAQ item clickable
    const wrapper = item.querySelector(".petgrooming-faq-answer-wrapper");
    const icon = item.querySelector(".petgrooming-faq-icon");
    ...
    trigger.addEventListener("click", () => {
      ...
      faqItems.forEach((other) => {
        ...
        if (other !== item) {
          ...
          const otherWrapper = other.querySelector(".petgrooming-faq-answer-wrapper");
          const otherIcon = other.querySelector(".petgrooming-faq-icon");
